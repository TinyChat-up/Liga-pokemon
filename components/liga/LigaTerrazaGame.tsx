...
export function LigaTerrazaGame() {
  ...
  function startEliteFour(){if(!finalUnlocked){setMessage("Completa los 12 QR antes de entrar en Meseta Añil.");return;}if(active.energy<=0){showCriticalAlert("energy","Tu Pokémon no tiene vida","Ve al Centro Pokémon de Alejandro antes de entrar en el Alto Mando.");return;}setEliteIndex(0);setEliteAnswer(null);setEliteOutcome(null);setChampionName("");setBattlePerkUsed(false);setBlockedAnswer(getGlaceonBlockedAnswer(active,eliteFourQuestions[0]));setEliteActive(true);setMessage("El Alto Mando comienza. Son 3 combates seguidos: no puedes salir hasta vencer o caer.");}
