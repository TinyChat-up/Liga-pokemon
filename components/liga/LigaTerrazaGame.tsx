...
export function LigaTerrazaGame() {
  ...
  async function loadGame(gameCode = activeGameCode){
    ...
    try {
      const snapshot = await loadGameSnapshot(supabase, gameCode);
  ...
  function selectPlayer(player: Player){setActiveId(player.id);localStorage.setItem(getActivePlayerStorageKey(activeGameCode),player.id);setScreen(player.evolution?(isOpen?"home":"waiting"):"partner");}
  ...
  function unlockAdmin(){if(!isMaster){setMessage("Solo el dispositivo master de esta partida puede abrir este panel.");return;}setMessage("");setScreen("admin");}
  ...
  return <main className={`app-shell ${playerHit?"damage-hit":""}`}><header className="topbar"><button className="wordmark" onClick={()=>{closeCamera();setScreen(activeId?(isOpen?"home":"waiting"):"select")}}><span className="ball-mark">◓</span> QR QUEST</button><div className="topbar-actions">{isMaster&&<span className="game-code-pill">{activeGameCode}</span>}{activeId&&<button className="admin-link" onClick={()=>{closeCamera();setScreen("select")}}>Perfil</button>}{isMaster&&<button className="admin-link" onClick={()=>{closeCamera();unlockAdmin();}}>Master</button>}<button className="admin-link" onClick={leaveGame}>Partida</button></div></header>
    ...
    {visibleScreen==="admin"&&<section className="panel-view admin-view"><button className="back" onClick={()=>{closeCamera();setScreen(activeId?(isOpen?"home":"waiting"):"select")}}>← Salir del panel master</button><p className="eyebrow">PANEL MASTER · {activeGameCode.toUpperCase()}</p><h2>Entrenadores</h2><p className="lead">Consulta jugadores creados, corrige evolución, borra perfiles y procesa curas, tokens o canjes.</p><button className="cantina-shortcut" onClick={()=>document.getElementById("admin-cantina")?.scrollIntoView({behavior:"smooth"})}>Abrir escáner de Cantina</button>{players.length?<div className="admin-roster">{players.map((player)=>{const evo=eeveelutions.find((item)=>item.name===player.evolution);return <article key={player.id} className="roster-row"><img src={`https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/other/official-artwork/${evo?.image??"133"}.png`} alt={evo?.name??"Eevee"}/><div><b>{player.name}</b><small>{evo?`${evo.name} · ${evo.type}`:"Aún no ha elegido"}</small></div><select aria-label={`Evolución de ${player.name}`} value={player.evolution??""} onChange={(e)=>e.target.value&&setPlayerEvolution(player.id,e.target.value as Evolution)}><option value="">Sin elegir</option>{eeveelutions.map((item)=><option key={item.name} value={item.name}>{item.name}</option>)}</select><button className="delete-profile" disabled={busyAction} onClick={()=>{void deletePlayerProfile(player);}}>Borrar</button></article>})}</div>:<p className="empty-admin">Aún no hay jugadores. Cada participante debe crear su perfil desde la pantalla inicial.</p>}<button className="danger-action" disabled={busyAction||!players.length} onClick={()=>{void resetCurrentGame();}}>Borrar todos los perfiles de esta partida</button><h3 id="admin-cantina">Centro Pokémon y barra</h3><div className="admin-card-scanner"><div><b>Identificar tarjeta</b><small>Escanea el QR virtual del perfil del jugador.</small></div><button className="camera-button" onClick={cameraOpen?closeCamera:openCamera}>{cameraOpen?"Cerrar cámara":"Escanear tarjeta"}</button></div>{cameraOpen&&<div className="arena-camera"><video ref={videoRef} playsInline muted/></div>}<div className="code-entry admin-code-entry"><input value={adminCardCode} onChange={(e)=>setAdminCardCode(e.target.value)} placeholder="Ej. player-ab12cd34" autoCapitalize="none" autoCorrect="off"/><button onClick={()=>selectAdminPlayer(adminCardCode)}>Buscar</button></div><select className="large-select" value={players.some((p)=>p.id===adminTarget)?adminTarget:""} onChange={(e)=>setAdminTarget(e.target.value)}>{players.length?players.map((p)=><option key={p.id} value={p.id}>{p.name}</option>):<option value="">Sin jugadores</option>}</select><article className="admin-player"><div className="mini-orb">◒</div><div><b>{players.length?target.name:"Sin jugador seleccionado"}</b><small>{target.tokens} tokens · energía {target.energy}% · ruta {target.route.length}/12</small></div></article><div className="admin-actions"><label className="heal-price"><span>Precio cura</span><input type="number" min="0" value={healCost} onChange={(e)=>setHealCost(Number(e.target.value))}/></label><button disabled={busyAction||!players.length} onClick={()=>{void heal();}}>Curar al 100%</button><button disabled={!players.length} onClick={award}>Dar +{REWARDS.arena} por Arena</button></div><h3>Canjear en barra</h3><div className="menu-grid">{menu.map((item)=><button disabled={busyAction||!players.length} key={item.label} onClick={()=>redeem(item)}><b>{item.label}</b><span>{item.cost} tokens</span></button>)}</div>{message&&<p className="toast">{message}</p>}</section>}
