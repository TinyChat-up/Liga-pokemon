...
export function LigaTerrazaGame() {
  ...
  useEffect(() => { const timer=window.setTimeout(()=>{const params=new URLSearchParams(location.search); const qr=params.get("qr")?.trim().toLowerCase(); const card=params.get("player"); const found=stations.find((item)=>item.id===qr); if(found) setQueuedStationId(found.id); if(qr==="arena") setScreen("arena"); if(card) { setPlayerCardCode(card); setMessage("Tarjeta de entrenador detectada. Elige tu perfil y abre la Arena o Team Rocket."); } if(qr==="alto-mando") setQueuedStationId("alto-mando");},0); const loadTimer=window.setTimeout(()=>{void loadGame();},0); const channel=subscribeToGameChanges(supabase,()=>{void loadGame();}); return()=>{window.clearTimeout(timer);window.clearTimeout(loadTimer);if(channel&&supabase)void supabase.removeChannel(channel);}; }, []);
  ...
  useEffect(()=>{if(screen==="admin"||screen==="admin-gate")return;const timer=window.setTimeout(()=>setAdminUnlocked(false),0);return()=>window.clearTimeout(timer);},[screen]);
  ...
  function unlockAdmin(){if(adminCode.trim()!==ACCESS_CODE){setMessage("Código de administrador incorrecto.");return;}setAdminUnlocked(true);setAdminCode("");setMessage("");setScreen("admin");}
