...
export function LigaTerrazaGame() {
  ...
  const visibleScreen: GameScreen = screen==="select" ? "select" : !activeId ? "select" : !active.evolution ? "partner" : isOpen ? screen : "waiting";
  ...
  return <main className="app-shell"><header className="topbar"><button className="wordmark" onClick={()=>setScreen(activeId?(isOpen?"home":"waiting"):"select")}><span className="ball-mark">◓</span> NIVEL <em>27</em></button>{activeId&&<button className="admin-link" onClick={()=>setScreen("select")}>Cambiar perfil</button>}</header>
    ...
    {(rouletteSpinning||rouletteResult)&&<div className="modal-wrap"><section className="roulette-modal"><p className="eyebrow">RULETA SORPRESA</p><h2>{rouletteSpinning?"Girando...":"Premio"}</h2><div className={`roulette-wheel ${rouletteSpinning?"spinning":""}`}><span>iPhone 17 Pro</span><span>Reloj inteligente</span><span>Viaje sorpresa</span><span>100 tokens</span><span>Prueba otra vez</span><span>Chupito</span></div><b>{rouletteResult||"¿Qué tocará?"}</b>{rouletteResult&&<button onClick={()=>{setRouletteResult("");setRouletteSpinning(false);}}>Cerrar</button>}</section></div>}
